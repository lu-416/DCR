...
def cal_global_nov(train_records, num_items):
    pop = [0] * num_items
    if num_items == 3883:
        sens = [0] * (len(train_records) + 2)
    else:
        sens = [0] * (len(train_records) + 1)
    ...
    for user in train_records:
        ...
        total_pop = sum(pop[item] for item in user_items)
        conformity = total_pop / len(user_items) if len(user_items) > 0 else 0
        sens[user] = conformity

    return 0, torch.tensor(pop), torch.tensor(sens)
...
def cal_local_nov(dataset, sim_users, train_records, num_items):
    ...
    if os.path.exists(f'/xx/co_items/local_nov-{dataset}-{sim_coe}.npy'):
        local_nov = np.load(f'/xx/co_items/local_nov-{dataset}-{sim_coe}.npy', allow_pickle=True)
    if os.path.exists(f'/xx/co_items/local_pop-{dataset}-{sim_coe}.npy'):
        local_pop = np.load(f'/xx/co_items/local_pop-{dataset}-{sim_coe}.npy', allow_pickle=True)
    if local_nov is None or local_pop is None:
        u_num = max(train_records.keys()) + 1
        print('user count: ', u_num)
        ...
        np.save(f'/xx/co_items/local_nov-{dataset}-{sim_coe}.npy', local_nov)
        np.save(f'/xx/co_items/local_pop-{dataset}-{sim_coe}.npy', local_pop)
